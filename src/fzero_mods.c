...
static const char *const packages[] = {"fzero-widescreen", "fzero-presentation-fps"};
...
static int package_get(void *ctx, int index, RecompLauncherCModPackage *out) {
  (void)ctx;
  if (index < 0 || index > 1 || !out) return 0;
  memset(out, 0, sizeof(*out));
  COPY(out->id, packages[index]); COPY(out->version, "1");
  COPY(out->name, names[index]); COPY(out->author, "FZeroSNESRecomp contributors");
  COPY(out->description, descriptions[index]);
  out->enabled = index ? video->fps_enabled : video->enhanced;
  return 1;
}
static int feature_get(void *ctx, int index, RecompLauncherCModFeature *out) {
  ...
  COPY(out->name, names[index]); COPY(out->group, "Presentation");
...
static int enable(void *ctx, const char *package, const char *feature, int enabled) {
  ...
  if (identity(package, feature) == 2) video->fps_enabled = enabled != 0;
  else {
